...
export async function logXp(delta: number, source: string) {
  const log = await loadXpLog();
...
export async function logSkillEvent(asId: string, type: 'review' | 'mastered') {
  const log = await loadSkillLog();
  const nextId = log.length > 0 ? log[log.length - 1].id + 1 : 1;
